...
_Figure 4.6.1.1: Component interactions for `delete-ingredient apple` command_
...
_Figure 4.7.1: Flow of events after a user executes the `stockup` command_
...
_Figure 4.7.3.1: DiscountItem Activity diagram_
...
_Figure 4.8.2.1: Activity diagram to illustrate the `add-reservation` command_
...
_Figure 4.8.5.1: Sequence diagram to illustrate the `list-reservations` command_
...
.. Test case: `clear-menu` +
   *Expected*: Deletes all items in the menu.
...
. Giving discount to all item

.. Prerequisites: You must have at least 1 item in the menu.
.. Test case: `discount-item ALL dp/20` +
   *Expected*: Gives all items in the menu a discount. Number of items given a discount shown in status message.
...
.. Prerequisites: The reservation to be edited must be an existing entry in the reservations list and the user must be
                  logged in.
...
.. Prerequisites: The reservation to be deleted must be an existing entry in the reservations list and the user must be
                  logged in.
